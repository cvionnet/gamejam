...
#region USER METHODS
    ...
    public PinMap Add_Instance(int pIndexSceneToDisplay, Vector2 pGlobalPosition, string pGroupName="")
    {
        ...
        // Instance
        PinMap instance;
        if (scene.Instance().GetType() == typeof(PinMap))
        {
            instance = (PinMap)scene.Instance();
            AddChild(instance);
    ...
    /// <param name="pGlobalPosition">Where to display the scene spawned</param>
    /// <param name="pGroupName">Name of the group the instance will belong</param>
    public Card Add_InstanceCard(int pIndexSceneToDisplay, Vector2 pGlobalPosition, string pGroupName="")
    {
        if(ListScenes == null || ListScenes.Count == 0 || ListScenes[pIndexSceneToDisplay] == null)
            return null;
        ...
        // Instance
        Card instance;
        if (scene.Instance().GetType() == typeof(Card))
        {
            instance = (Card)scene.Instance();
            AddChild(instance);
    ...
    /// <param name="pGroupName">Name of the group the instance will belong</param>
    async public Task<PinMap> Add_Instance(Vector2 pGlobalPosition, Spawn_Timing pTiming, int pSpawnNumber=1, bool pRandomInstance=false, string pGroupName="")
    {

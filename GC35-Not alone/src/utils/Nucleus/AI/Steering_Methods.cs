...
namespace Nucleus.AI
{
    public partial class Nucleus_Steering
    {
        ...
        private const float STEERING_DEFAULT_FOLLOW_OFFSET = 60.0f;    // to add an offset (=distance) between following nodes
        
        /// <summary>
        /// Set the next position where the agent have to go
        /// </summary>
        public void Set_TargetGlobalPosition(Vector2 nextPosition)
            => TargetGlobalPosition = nextPosition;
        
        /// <summary>
        /// Set the next position where the agent have to go, within limits
        /// </summary>
...
//                            pCharacterProperties.Steering.Slow_Radius, pCharacterProperties.Steering.Mass, pStopRadius);
        ...
        public Vector2 Steering_CalculateDistanceBetweenFollowers(Vector2 pLeaderPosition, Vector2 pFollowerPosition, float pFollowOffset = STEERING_DEFAULT_FOLLOW_OFFSET)
        {
